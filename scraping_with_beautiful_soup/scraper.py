...
def get_product_details(soup):
    ...
    location = (
        soup.select_one("p.detail-sub-heading i").find_parent("a").get_text(strip=True)
    )

    model_year = soup.find("span", class_="engine-icon year").parent.get_text(
        strip=True
    )

    mileage = soup.find("span", class_="engine-icon millage").parent.get_text(
        strip=True
    )

    seller_name_data = (
        soup.find("div", class_="owner-detail-main")
    ...
    cover_image_url = (
        all_images[1]["data-original"]
        if len(all_images) > 1
        else all_images[0]["data-original"]
    )
...
def get_product_urls(page_urls, first_page_soup=None):
    ...
    for index, url in enumerate(page_urls):
        soup = (
            first_page_soup
            if index == 0 and first_page_soup
            else get_soup_by_selenium_driver(url)
        )
